...
//                psst.setString(count++, user.getSalt());
    ...
    public User updateUser(User user) {
        String sql = "update sys_user set organization_id=?,username=?, password=?,   role_ids=?, locked=? where id=?";
        jdbcTemplate.update(
                sql,
                user.getOrganizationId(), user.getUsername(), user.getPassword(), user.getRoleIdsStr(), user.getLocked(), user.getId());
        return user;
    }
    ...
    public User findByNo(String no) {
    	String sql = "select id, organization_id,no, username, password, salt, role_ids as roleIdsStr, locked from sys_user where no=?";
    	List<User> userList = jdbcTemplate.query(sql, new BeanPropertyRowMapper(User.class), no);
    	if(userList.size() == 0) {
    		return null;
    	}
    	return userList.get(0);
